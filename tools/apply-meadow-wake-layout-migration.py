"""One-use transfer of the tested prop-grounding corrections."""
from pathlib import Path
import hashlib
import json
root = Path(__file__).resolve().parents[1]
allowed = {'src/game.js', 'src/character-renderer.js', 'src/environment/meadow-wake-foreground.js'}
entries = json.loads((root / 'tools/meadow-wake-grounding-transfer.json').read_text())
if len(entries) != len(allowed) or {entry['path'] for entry in entries} != allowed:
    raise RuntimeError('Invalid transfer inventory')
writes = []
